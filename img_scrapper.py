...
def search_and_download(search_term, number_images, driver=driver, target_path='./images'):
    ...
    global glob_time 
    glob_time_ini = time.time()
    glob_time = str(glob_time_ini)
...
def delunwantedzip():
    global glob_time
    glob_time_float = float(glob_time)
    for name in glob.glob('static/*_*.zip'):
        if os.stat(name).st_mtime < glob_time_float - 5*60:
...
def delunwantedimgs():
    global glob_time
    glob_time_float = float(glob_time)
    for name in glob.glob('images/*_*'):
        if os.stat(name).st_mtime < glob_time_float - 5*60:
